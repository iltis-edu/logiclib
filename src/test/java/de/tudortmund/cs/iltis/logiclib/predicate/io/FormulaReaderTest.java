...
public class FormulaReaderTest extends TestRig<Formula> {
    @BeforeClass
    public static void initTestRig() {
        checkForAllowedVars = false;
        checkForNegativeTarget = false;

        PredicateReaderProperties props = PredicateReaderProperties.createDefault();
        props.setSymbolSplittingPolicy(
                RegularSymbolSplittingPolicy.UNARY_NAME_AND_INDEX_POLICY.or(
                        RegularSymbolSplittingPolicy.CREATE_OF_SYMBOLS("###", "~_1")));
        props.getInfixProperties().addInfixFunctionsSymbols("~_1");
        props.getInfixProperties().addInfixRelationSymbols("###");
        assertTrue("properties not consistent", props.isConsistent());
        ...
        // signature 3: (E/2)
        Set<RelationSymbol> rels3 = new TreeSet<>();
        rels3.add(new RelationSymbol("E", 2, false));
        Set<FunctionSymbol> funs3 = new TreeSet<>();
        Signature signature3 = new Signature(rels3, funs3);
        ...
        SignaturePolicy extPolicy =
                SignaturePolicy.TU_DORTMUND_LS1_WITH_INDEX_POLICY
                        .weakenFunctionConstraints(new FunctionSymbol("~_1", 2, true))
                        .weakenRelationConstraints(new RelationSymbol("###", 2, true));
        ...
        positives.add(
                new Object[] {
                    "exists xE(x,t^a)",
                    new RelationAtom("E", new Variable("x"), new Variable("t^a"))
                            .exists(new Variable("x")),
                    sigPolicy,
                    vars
                });
        // missing space
        positives.add(
                new Object[] {
                    "exists xE(x,t^'all')",
                    new RelationAtom("E", new Variable("x"), new Variable("t^all"))
                            .exists(new Variable("x")),
                    sigPolicy,
                    vars
                });
        ...
        positives.add(
                new Object[] {
                    ...
                    new RelationAtom("R", new Variable("x^1_a")).exists(new Variable("x_a^1")),
                    sigPolicy,
                    vars
                });
        ...
        positives.add(
                new Object[] {
                    "exists x^1_'12a' x^1_'12a'###(z_2~_1b^1_'test')",
                    new RelationAtom(
                                    "###",
                                    true,
                                    new Variable("x^1_12a"),
                                    new FunctionTerm(
                                            "~_1",
                                            true,
                                            new Variable("z_2"),
                                            new FunctionTerm("b_test^1")))
                            .exists(new Variable("x_12a^1")),
                    extPolicy,
                    vars
                });
        ...
        // rel symbol with wrong arity for given signature
        negatives.add(
                new Object[] {
                    "∃x(¬E())",
                    null,
                    ...
                    vars,
                    SubsetFaultCollection.class,
                    SubsetFaultReason.ARITY_OF_RELATION_SYMBOL_WRONG
                });
        ...
        negatives.add(
                new Object[] {
                    "exists x^1_'12a' (x^1_'12a'###(A_2~_1b^1_'test'))",
                    new RelationAtom(
                                    "###",
                                    true,
                                    new Variable("x^1_12a"),
