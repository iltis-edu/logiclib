...
public class NRestriction implements PredicateResolutionRestriction {

    // needed for serialization
    private NRestriction() {}
     ...
     */
    public NRestriction(Graph<DisjunctiveClause, Substitution> graph, boolean checkSingleSteps) {

        this.checkSingleSteps = checkSingleSteps;
        this.baseNodes = graph.getVertexValues();
    }

    public NRestriction(Formula formula, boolean checkSingleSteps) {
        this.checkSingleSteps = checkSingleSteps;
    ...
    public boolean isSatisfied(
        ...
        return !containsPositiveLiterals(firstClause) || !containsPositiveLiterals(secondClause);
    }
    ...
    public boolean isSatisfied(Graph<DisjunctiveClause, Substitution> graph) {
        for (DisjunctiveClause clause : graph.getVertexValues()) {
            if (!baseNodes.contains(clause)) {
                ...
                if (containsPositiveLiterals(parents.first())
                        && containsPositiveLiterals(parents.second())) {
    ...
    private boolean containsPositiveLiterals(DisjunctiveClause first) {
