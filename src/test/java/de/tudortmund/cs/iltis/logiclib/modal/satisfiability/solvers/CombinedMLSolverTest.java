...
public class CombinedMLSolverTest {
    ...
    // @Test
    public void testSimpleEquivalences() {
        formula1 = ModalFormula.parse("☐(L∨M∨P)∧◇B");
        formula2 = ModalFormula.parse("☐(L∨M∨P)∧¬◇¬B");
        testEquivalence(AnswerType.INVALID);

        formula1 = ModalFormula.parse("◇◇¬D∨◇¬A");
        formula2 = ModalFormula.parse("◇◇¬D∨¬☐A");
        testEquivalence(AnswerType.VALID);

        formula1 = ModalFormula.parse("¬(E∧¬☐A)∨¬(◇(K∧☐A)↔☐F)");
        formula2 = ModalFormula.parse("(E∧¬☐A)→¬(◇(K∧☐A)↔☐F)");
        testEquivalence(AnswerType.VALID);

        Transformation nnfTransformation = EquivalenceTransformations.NEGATION_NORMALFORM;

        formula1 = ModalFormula.parse("(E∧¬☐A)→¬(◇(K∧☐A)↔☐F)");
        formula2 = nnfTransformation.apply(ModalFormula.parse("(E∧¬☐A)↔¬(◇(K∧☐A)↔☐F)"));
        testEquivalence(AnswerType.INVALID);

        formula1 = ModalFormula.parse("(E∧¬☐A)");
        formula2 = nnfTransformation.apply(ModalFormula.parse("(E∧◇¬A)"));
        testEquivalence(AnswerType.VALID);
    }

    private void testEquivalence(AnswerType type) {
        CombinedMLSolver solver = new CombinedMLSolver();
