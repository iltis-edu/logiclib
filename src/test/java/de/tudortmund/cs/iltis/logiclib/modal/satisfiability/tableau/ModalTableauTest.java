...
public class ModalTableauTest extends AdvancedTest {
    ...
    public void simplePropositional() {
        ...
        ModalTableau tableau = new ModalTableau(psi);
    ...
    public void bugTest() {
        try {
            ModalTableau tableau =
                    new ModalTableau(formulaReader.read("(dia A | B) & dia C & box ! B"), false);

            FormulaNode current = tableau.root;
            tableau.saturate(current);
            current = (FormulaNode) current.getChild(0);
            tableau.saturate(current);
            current = (FormulaNode) current.getChild(0);
            tableau.saturate(current);
            current = (FormulaNode) current.getChild(0);
            FormulaNode box = current;
            tableau.saturate(current);
            current = (FormulaNode) current.getChild(0);
            tableau.saturate(current);
            tableau.saturate(box);
            tableau.isOpen();
