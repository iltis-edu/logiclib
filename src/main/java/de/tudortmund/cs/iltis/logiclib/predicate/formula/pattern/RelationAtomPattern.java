...
/**
 * Matches any FO formula consisting of only a single {@link RelationAtom}.
 *
 * @see TreePattern
 */
public class RelationAtomPattern extends MultiConstraintPattern<TermOrFormula> {

    // needed for serialization
    private RelationAtomPattern() {}

    private static PredicatePattern<TermOrFormula> createPredicatePattern() {

        return new PredicatePattern<>(forest -> forest.size() == 1 && forest.get(0).isRelation());
    }

    public RelationAtomPattern(
    ...
    public TermOrFormula createTree(@NotNull final Match<TermOrFormula> match) {
        try {
            ...
            if (optTree.isPresent()) {
                ...
                return new RelationAtom(optName.get().toString(), subterms);
            }
        }
    }
    ...
    public PatternType getType() {
        return FOPatternType.RelationAtomPattern;
    }

    @Override
    public String toString() {
        return "RelationAtomPattern [id=" + name + ", subpatterns=" + children + "]";
    }

    @Override
    public RelationAtomPattern clone() {
        return new RelationAtomPattern(name, eqTester, getClonedChildren());
    }

    @Override
    public RelationAtomPattern cloneWithIteratedName(int iteration) {
